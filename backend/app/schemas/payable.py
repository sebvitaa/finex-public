...
PayableStatus = Literal["pending_payment", "partially_paid", "paid", "overdue", "forgiven"]
...
class PayableBase(BaseModel):
    ...
    status: PayableStatus = "pending_payment"
    notes: str | None = None
    ...
    def default_remaining_amount(self) -> "PayableBase":
...
class PayablePaymentAllocationCreate(BaseModel):
    payable_id: int
    amount: Decimal = Field(gt=0, max_digits=12, decimal_places=2)
...
class PayablePaymentBatchCreate(BaseModel):
    transaction_id: int | None = None
    paid_at: datetime
    payments: list[PayablePaymentAllocationCreate] = Field(min_length=1)
    notes: str | None = None


class PayablePaymentRead(PayablePaymentCreate):
    id: int
    payable_id: int
    created_at: datetime
...
class PayableRead(PayableBase):
    id: int
    person: PersonRead
    payments: list[PayablePaymentRead] = Field(default_factory=list)
    created_at: datetime
    updated_at: datetime
